...
def get_wan_ip():
    try:
        ...
        res.configure(text='公网 IP  : ' + wan_ip, fg='#600')
    except:
...
def get_local_ip():
    try:
        ...
        res.configure(text='局域网 IP  : ' + lan_ip, fg='#600')
    except:
...
def about():
    ...
    close_app = Button(root, text='退出', command=close_about, bg='#55F')
...
def close_about():
    ...
    frame.destroy()
    close_app.destroy()
    about_app = Button(root, text='关于', command=about)
...
res_wan_ip = Button(root, text='公网 IP', command=get_wan_ip)
res_local_ip = Button(root, text='局域网 IP', command=get_local_ip)
about_app = Button(root, text='关于', command=about)
quit_app = Button(root, text='退出', command=quit, bg='#f40')
